...
#        if self.__now_pointlist_index < len(self.__pointlist):
    ...
    def __move(self):
        ...
        if 0 == self.__frame_target:
            self.__now_pointlist[-1][0] = int(self.__pointlist[self.__now_pointlist_index][0] * rate)
            self.__now_pointlist[-1][1] = int(self.__pointlist[self.__now_pointlist_index][1] * rate) 

#            self.__now_pointlist[-1][0] = int(self.__pointlist[self.__now_pointlist_index-1][0] + self.__pointlist[self.__now_pointlist_index][0] * rate)
#            self.__now_pointlist[-1][1] = int(self.__pointlist[self.__now_pointlist_index-1][1] + self.__pointlist[self.__now_pointlist_index][1] * rate) 
#            self.__now_pointlist[-1][0] = int(self.__pointlist[self.__now_pointlist_index][0] * rate * self.__anime_direct_x)
#            self.__now_pointlist[-1][1] = int(self.__pointlist[self.__now_pointlist_index][1] * rate * self.__anime_direct_y) 
        else:
            self.__now_pointlist[-1][0] = int(self.__pointlist[self.__now_pointlist_index][0] * rate)
            self.__now_pointlist[-1][1] = int(self.__pointlist[self.__now_pointlist_index][1] * rate)
#            self.__now_pointlist[-1][0] = int(self.__pointlist[self.__now_pointlist_index][0] * rate * self.__anime_direct_x)
...
#        self.__frame_target = 1 if diff_x < diff_y else 0
#        self.__frame_max = abs(diff_x) if diff_x < diff_y else abs(diff_y)
